...
export default function Fornecedores() {
  ...
  const fetchData = useCallback(async (override = {}) => {
    ...
    try {
      const params = {
        ...
        per_page: rows,
        page: page + 1,
        ...override, // permite forçar page=1 em buscas
      };

      const { items: list, meta } = await listarFornecedores(params, { signal: controller.signal });
      setItems(list || []);
      setTotal(meta?.total ?? list?.length ?? 0);
    } catch (e) {
      // axios abort: e.name === 'CanceledError' ou e.code === 'ERR_CANCELED'
      const aborted = e?.name === 'CanceledError' || e?.code === 'ERR_CANCELED' || e?.message === 'canceled';
      if (!aborted) {
        console.error(e);
        toast.current?.show({ severity: 'error', summary: 'Erro', detail: 'Falha ao carregar fornecedores.' });
      }
    } finally {
      setLoading(false);
    }
  ...
  // Busca inicial e quando filtros (exceto q) mudam
  useEffect(() => {
    fetchData();
  }, [page, rows, sortField, sortOrder, status, withTrashed, fetchData]);

  // Debounce para 'q'
  useEffect(() => {
    const t = setTimeout(() => {
      // ao digitar, volta para a primeira página e busca
      setPage(0);
      fetchData({ page: 1 });
    }, 400);
  ...
  // cleanup: aborta requisição ao desmontar
  ...
  const onSubmitForm = async (payload) => {
    ...
    try {
      if (editing?.id) {
        ...
        toast.current?.show({ severity: 'success', summary: 'Fornecedor', detail: 'Atualizado com sucesso.' });
      } else {
        await criarFornecedor(payload);
        toast.current?.show({ severity: 'success', summary: 'Fornecedor', detail: 'Criado com sucesso.' });
      }
      setFormVisible(false);
      setEditing(null);
      // recarrega lista na primeira página após salvar
      setPage(0);
      await fetchData({ page: 1 });
    } catch (err) {
      console.error(err);
      const msg = err?.response?.data?.message || 'Falha ao salvar fornecedor.';
  ...
  const remover = async (row) => {
    try {
      await excluirFornecedor(row.id);
      toast.current?.show({ severity: 'success', summary: 'Fornecedor', detail: 'Removido (soft delete).' });
      // mantém página atual, só atualiza dados
      fetchData();
    } catch (err) {
      console.error(err);
  ...
  const restaurar = async (row) => {
    try {
      await restaurarFornecedor(row.id);
      toast.current?.show({ severity: 'success', summary: 'Fornecedor', detail: 'Restaurado com sucesso.' });
      fetchData();
    } catch (err) {
      console.error(err);
  ...
  const canCriar   = has(PERMISSOES.FORNECEDORES?.CRIAR);
  const canEditar  = has(PERMISSOES.FORNECEDORES?.EDITAR);
  const canExcluir = has(PERMISSOES.FORNECEDORES?.EXCLUIR);

  const confirmarExclusao = (event, row) => {
    confirmPopup({
      target: event.currentTarget,
      message: `Remover o fornecedor "${row?.nome}"?`,
  ...
  const cnpjBody = (row) => <span>{formatCNPJ(row.cnpj)}</span>;
  const produtosCountBody = (row) => <span className="font-semibold">{row.produtos_count ?? 0}</span>;
  const acoesBody = (row) => {
  ...
  return (
    <SakaiLayout>
      ...
      <div className="surface-section p-4 md:p-6">
        <h2 className="text-2xl mb-3">Fornecedores</h2>

        <Panel header="Gestão de Fornecedores" toggleable collapsed={false} className="mb-3">
          <p className="m-0 text-600">Cadastre e gerencie fornecedores/fabricantes.</p>
        </Panel>
        ...
        <div className="card">
          ...
          >
            ...
            <Column field="cnpj" header="CNPJ" body={cnpjBody} style={{ minWidth: '170px' }} />
            ...
            <Column field="produtos_count" header="Produtos" body={produtosCountBody} sortable style={{ width: '120px', textAlign: 'right' }} />
            <Column
              field="updated_at"
